...
# Endpoint pour obtenir des films aléatoires au lancement (fixés pour une session)
@app.route('/random_movies', methods=['GET'])
def get_random_movies():
    # Vérifiez si des films sont déjà dans la session
    if 'random_movies' not in session:
        random_movies = random.sample(list(movies['id']), 5)  # 5 films aléatoires
        session['random_movies'] = random_movies  # Sauvegarde dans la session
    
    # Récupérez les détails des films stockés dans la session
    random_movies = session['random_movies']
